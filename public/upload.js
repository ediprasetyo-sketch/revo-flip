...
async function blobToBase64(blob) { const buffer = await blob.arrayBuffer(); const bytes = new Uint8Array(buffer); const step = 0x8000; let binary = ''; for (let i = 0; i < bytes.length; i += step) binary += String.fromCharCode(...bytes.subarray(i, i + step)); return btoa(binary); }
function selectFile(f) {
  if (!f) return;
  if (f.type && f.type !== 'application/pdf' && !/\.pdf$/i.test(f.name)) { status.textContent = 'Hanya file PDF yang didukung.'; return; }
  if (f.size > MAX_FILE_SIZE) { status.textContent = 'Ukuran file melebihi batas 10 GB.'; return; }
  file = f;
  nameEl.textContent = f.name;
  sizeEl.textContent = formatSize(f.size);
  info.hidden = false;
  status.textContent = localStorage.getItem(key(f)) ? 'Sesi upload sebelumnya ditemukan. Klik Mulai Upload untuk melanjutkan.' : 'Siap diunggah.';
}
  ...
  try {
    ...
    for (let part = 1; part <= totalParts; part++) {
      ...
      await retry(async () => {
        ...
        const r = await fetch(`/api/upload/part?uploadId=${encodeURIComponent(session.uploadId)}&part=${part}`, {
          method: 'PUT',
    ...
    status.innerHTML = `Upload berhasil. <a href="${result.viewer}">Buka Flipbook</a>`;
  } catch (err) {
    status.textContent = `Upload terhenti: ${err.message}. Pilih file yang sama dan klik Mulai Upload untuk melanjutkan.`;
