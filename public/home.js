...
function selectFile(f){
  ...
  panel.hidden=false;
  bar.style.width='0%';
  progress.textContent=localStorage.getItem(key(f))?'Sesi sebelumnya ditemukan. Klik Mulai Upload untuk melanjutkan.':'Siap diunggah.';
  status.textContent='';
}
...
start.addEventListener('click',async()=>{
  ...
  try{
    ...
    setTimeout(()=>location.href=result.viewer,350);
  }catch(err){
    status.textContent=`Upload terhenti: ${err.message}. Klik tombol Upload File PDF dan pilih file yang sama untuk melanjutkan.`;
    pick.disabled=false;
